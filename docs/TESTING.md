...
| Default consistency (8.2.4)                                       | In `npm run check`                | 30 plugin default classes plus the 5 inline variables the plugin writes on `body`, compared against the no-plugin state. Reverting the fix reports `--file-line-width is 700px without the plugin and 760px with it` and the same for `--line-height-normal`                                                                                                                                                                                                                                                       |
| Mobile control surfaces (G03)                                     | In `npm run check`                | Native `.is-mobile.theme-dark` in the contract, then the desktop and phone states compared token by token. Removing the platform block reports `--interactive-hover is #394b58 on the desktop and #7fc3e8 on a phone; the border accent must not become a control fill`. Browser measurement: an enabled button's hover text was 1.60:1 and is 7.51:1                                                                                                                                                              |
| Disabled control states (G04)                                     | In `npm run check`                | Real properties on synthetic fields, read through the harness's pseudo-state sets. Declaring the disabled block before the enabled rules reports `a disabled field must not carry a resting shadow: box-shadow resolved to list, expected none`, the same on hover, and `background-color moves from #191f26 to #2c343c on hover`. Browser measurement: opacity 0.5 to 0.68                                                                                                                                        |
| Mobile destructive levels (G03c)                                  | In `npm run check`                | The primary and secondary delete buttons must resolve to different surfaces on a phone. Dropping the `:where(:not(.mod-cta))` guard reports `the secondary and primary delete buttons share #3b1f2a; the two destructive levels must stay distinct on a phone`                                                                                                                                                                                                                                                     |
| Control shape roles (G01/G02)                                     | In `npm run check`                | Button, field and icon corners resolved as real properties in four states. Reverting the consumption reports `the button rounds at 8px but its role is 10px` and `the icon rounds at 4px but its role is 8px`; moving the icon declaration into the mode layer reports `the icon corner is 8px; the platform's 44px touch radius must reach it`                                                                                                                                                                    |
| Selector crosstalk (G05)                                          | In `npm run check`                | An icon resolves the same whether it is a `div` or a `button`, and a hover must not repaint a control the theme does not style. Restoring the bare `button` selector reports `background-color is <missing> on a div and #2c343c on a button`; restoring `input:hover` reports `hover repaints the background from <missing> to #2c343c` for checkbox, radio, range and colour                                                                                                                                     |
| Canvas tool group (G07)                                           | In `npm run check`                | The group rounds at the tool group role and the items keep their seams. Dropping the declaration reports `rounds at unresolved --canvas-controls-radius, expected the tool group role 12px`                                                                                                                                                                                                                                                                                                                        |
| Shadow roles (G06)                                                | In `npm run check`                | A field and a button no longer share a lift. The disabled-field guard asks that hovering an enabled field still changes something, so it cannot be satisfied by an over-broad guard. Boundary contrast is unchanged: the field border carries the 3:1 control floor                                                                                                                                                                                                                                                |
| Callout space levels (G09)                                        | In `npm run check`                | Quiet, default and Airy padding must be ordered on both axes. Reverting Airy to the spacing scale reports `vertical padding runs 8 / 18 / 16 for quiet / default / airy; the levels are out of order`                                                                                                                                                                                                                                                                                                              |
| Density and touch geometry (G08)                                  | In `npm run check`                | `--input-height` resolves to 32 / 34 / 38px across the three densities, **both** forms of `.text-icon-button` carry the same min-block-size at each level, a bare `.clickable-icon` stays unsized, and the three themed touch containers hold 44px on a phone. Setting Compact back to 34px reports `--input-height is 34px, expected 32px`; dropping the role reports `div.text-icon-button has min-block-size <missing>, expected 34px`; dropping a container to 40px reports it against the 44px project target |
| Motion scope (G10)                                                | In `npm run check` + browser      | The durations resolve to the theme's 120 / 80 / 0ms rather than native's 140 / 70 / 0. Measured with the native stylesheet loaded: `--anim-duration-fast` is `0.12s`. The reduced-motion guard is property-based, so it does not depend on these tokens                                                                                                                                                                                                                                                            |
| Writing direction (G11)                                           | In `npm run check` + browser      | The callout's inline-start corners are square and its inline-end corners round, and the wash direction reverses. Dropping the logical corners reports `border-start-start-radius resolves to <missing>`; removing the RTL override reports `the wash direction is to right in both directions; it must reverse`. Verified in a browser with `direction: rtl`: corners flip, wash flips                                                                                                                             |
| Quiet status bar (G12)                                            | In `npm run contrast` + browser   | Composed contrast, not the token value: 2.95:1 in light before, 4.52:1 after; dark 4.99 to 6.29. Both pairs are configured at a 4.5:1 minimum, so a change that erodes the margin fails                                                                                                                                                                                                                                                                                                                            |
| Mobile disabled destructive (combination)                         | In `npm run check`                | Mobile x destructive x disabled x hover, over four disabled spellings. Removing the disabled exclusion reports `disabled delete button on a phone: the fill moves from #3b1f2a to #623843 on hover; a disabled control must not answer the pointer` and the mobile-versus-desktop mismatch                                                                                                                                                                                                                         |
| Callout nesting (8.2.5)                                           | In `npm run check`                | A nested info Callout must resolve what standalone info resolves. Moving the type default onto the mode block reports "nested info resolves 5.00% but standalone info resolves 6.00%; nesting changed the type resolution". Also asserts code inside a Callout keeps `--code-border-width: 1px`                                                                                                                                                                                                                    |
...
## Callout icon integration — 2026-09-14

`npm run scenarios` now asserts 15 distinct SVG strings with child-group fill protection, default 22
px/28 px geometry, original/native stroke roles, all six compatibility aliases, and nested
standard-type stroke resets. The wash setting matrix includes the new names and `aoi-music`. Numeric
stroke comparisons tolerate the CSS parser's float32 representation.

A separate Chromium page loaded the generated `theme.css` after native Obsidian 1.13.7 CSS and
reproduced the observed SVG root normalization (16 px attributes plus currentColor fill/stroke). 858
assertions passed across light/dark and default/Quiet/Airy: original SVG XML parsing, computed 22 px
icons, 28 px circular tinted badges, unfilled child groups, 1.25 strokes, unchanged fold-chevron
size, opaque normal-blend containers, aliases, and nested type isolation. Both modes were visually
inspected. This is isolated rendering, not actual Obsidian interaction coverage.

Manual follow-up: open the examples in `docs/CALLOUTS.md` in actual Reading and Live Preview; check
fold/unfold and keyboard focus, nested/long-title/RTL notes, settings lifecycle, pop-outs, physical
mobile and forced-colors rendering. The local test vault's theme link points at another checkout, so
it was not silently repointed or claimed as a test of this build.
